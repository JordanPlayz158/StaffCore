...
public class ClearChat extends PaginatedMenu {
    ...
    public ClearChat(PlayerMenuUtility playerMenuUtility, StaffCorePlugin plugin) {
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        if (e.getCurrentItem().getItemMeta().getPersistentDataContainer().has(new NamespacedKey(plugin, "clearPlayer"), PersistentDataType.STRING)) {
            ...
            if (e.getClick().isLeftClick()) {
                new ChatSettings(StaffCorePlugin.getPlayerMenuUtility(p), StaffCorePlugin.plugin).open(p);
            }
        ...
        if (players != null && !players.isEmpty()) {
            for (int i = 0; i < getMaxItemsPerPage(); i++) {
                ...
                if (players.get(index) != null) {
                    ...
                    meta.setLore(lore);
                    meta.getPersistentDataContainer().set(new NamespacedKey(StaffCorePlugin.plugin, "clearPlayer"), PersistentDataType.STRING, "clearPlayer");
