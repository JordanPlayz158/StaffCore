...
public class openReportsMenu extends PaginatedMenu {
    ...
    public openReportsMenu(PlayerMenuUtility playerMenuUtility, StaffCorePlugin plugin) {
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        if (utils.mysqlEnabled()) {
            ...
            for (int id = 0; id <= utils.count("reports"); ) {
                id++;
                try {
                    if (plugin.reports.getConfig().getString("reports." + id + ".status").equals("open")) {
                        num++;
                        reports.put(num, id);
                    }
        ...
        if (e.getCurrentItem().getItemMeta().getPersistentDataContainer().has(new NamespacedKey(plugin, "open"), PersistentDataType.STRING)) {
            ...
            if (e.getClick().isLeftClick()) {
                ...
                new EditReport(StaffCorePlugin.getPlayerMenuUtility(p), plugin, jugador, id).open();
            ...
            for (int i = 1; i <= getMaxItemsPerPage(); i++) {
                ...
                if (reports.get(index) != null) {
                    ...
                    if (utils.mysqlEnabled()) {
                        ...
                        meta.setLore(lore);
                        meta.getPersistentDataContainer().set(new NamespacedKey(StaffCorePlugin.plugin, "open-id"), PersistentDataType.INTEGER, reports.get(index));
