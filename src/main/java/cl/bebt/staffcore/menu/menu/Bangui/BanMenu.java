...
public class BanMenu extends BanPlayerMenu {
    ...
    public BanMenu(PlayerMenuUtility playerMenuUtility, StaffCorePlugin plugin, Player p, String p2) {
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        for (String key : inventorySection.getKeys(false)) {
            ...
            if (e.getCurrentItem().getItemMeta().getPersistentDataContainer().has(new NamespacedKey(plugin, key), PersistentDataType.STRING)) {
                p.closeInventory();
                new ChoseBanType(StaffCorePlugin.getPlayerMenuUtility(p), plugin, baner, banned, reason).open();
            }
        }
        if (e.getCurrentItem().getItemMeta().getPersistentDataContainer().has(new NamespacedKey(plugin, "other"), PersistentDataType.STRING)) {
            ...
            PlayerData.set(new NamespacedKey(StaffCorePlugin.plugin, "banmsg"), PersistentDataType.STRING, banned);
