...
public class OpenEnderSee {
    ...
    public OpenEnderSee(Player p, Player target) {
        if (!StaffCorePlugin.enderSee.containsValue(target)) {
            StaffCorePlugin.enderSee.put(p, target);
        }
    ...
    public static Player getOwner(Player target) {
        Player owner = null;
        try {
            for (Map.Entry<Player, Player> players : StaffCorePlugin.enderSee.entrySet()) {
                if (StaffCorePlugin.enderSee.get(players.getKey()).equals(target)) {
