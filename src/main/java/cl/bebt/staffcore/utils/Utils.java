...
public class Utils {
    ...
    public Utils(StaffCorePlugin plugin) {
        Utils.plugin = plugin;
    }
    ...
    public static int currentPlayerWarns(String warned) {
        ...
        if (mysqlEnabled()) {
            ...
            StaffCorePlugin.plugin.warns.reloadConfig();
            for (int i = 1; i <= plugin.warns.getConfig().getInt("count"); i++) {
                try {
                    if (StaffCorePlugin.plugin.warns.getConfig().getString("warns." + i + ".name").equalsIgnoreCase(warned)) {
                        warnings++;
                    }
    ...
    public static int currentPlayerReports(String reported) {
        ...
        if (mysqlEnabled()) {
            ...
            StaffCorePlugin.plugin.reports.reloadConfig();
            for (int i = 1; i <= plugin.reports.getConfig().getInt("count"); i++) {
                try {
                    if (StaffCorePlugin.plugin.reports.getConfig().getString("reports." + i + ".name").equalsIgnoreCase(reported)) {
                        reports++;
                    }
                } catch (NullPointerException ignored) {
                }
