...
public class CountdownManager {
    ...
    public static void setMuteCountdown(Player p, long seconds) {
        ...
        if (checkMuteCountdown(p)) {
            ...
            persistent.set(new NamespacedKey(StaffCorePlugin.plugin, "muted_time"), PersistentDataType.DOUBLE, delay);
        } else {
            persistent.remove(new NamespacedKey(StaffCorePlugin.plugin, "muted_time"));
            double delay = System.currentTimeMillis() + (seconds * 1000);
            persistent.set(new NamespacedKey(StaffCorePlugin.plugin, "muted_time"), PersistentDataType.DOUBLE, delay);
        }
    }
    ...
    public static long getMuteCountDown(Player p) {
        PersistentDataContainer persistent = p.getPersistentDataContainer();
        double delay = persistent.get(new NamespacedKey(StaffCorePlugin.plugin, "muted_time"), PersistentDataType.DOUBLE);
        return Math.toIntExact(Math.round((delay - System.currentTimeMillis()) / 1000));
    }

    public static boolean checkMuteCountdown(Player p) {
        PersistentDataContainer persistent = p.getPersistentDataContainer();
        if (!persistent.has(new NamespacedKey(StaffCorePlugin.plugin, "muted_time"), PersistentDataType.DOUBLE) ||
                persistent.get(new NamespacedKey(StaffCorePlugin.plugin, "muted_time"), PersistentDataType.DOUBLE) <= System.currentTimeMillis()) {
            return true;
        }
        return !persistent.has(new NamespacedKey(StaffCorePlugin.plugin, "muted_time"), PersistentDataType.DOUBLE) ||
                persistent.get(new NamespacedKey(StaffCorePlugin.plugin, "muted_time"), PersistentDataType.DOUBLE) <= System.currentTimeMillis();
    }
