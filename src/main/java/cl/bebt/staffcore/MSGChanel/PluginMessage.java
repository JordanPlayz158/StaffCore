...
public class PluginMessage implements PluginMessageListener {
    private final StaffCorePlugin plugin = StaffCorePlugin.plugin;
    ...
    public void WarnAlert(String sender, String target, String reason, Long amount, String time, String ExpDate, String date, String server) {
        ...
        if (Bukkit.getPlayer(target) instanceof Player && utils.currentPlayerWarns(target) >= utils.getInt("warns.max_warns", null)) {
            ...
            utils.PlayParticle(Bukkit.getPlayer(target), "ban");
            if (StaffCorePlugin.plugin.getConfig().getBoolean("wipe.wipe_on_ban"))
                wipePlayer.WipeOnBan(StaffCorePlugin.plugin, target);
            String finalBan_msg = ban_msg;
            Bukkit.getScheduler().scheduleSyncDelayedTask(StaffCorePlugin.plugin, () -> Bukkit.getPlayer(target).kickPlayer(utils.chat(finalBan_msg)), 7L);
        }
    }
    ...
    public void openStaffChat(String sender, String server, int count, String staffMembers, String staffMembersServer, String staffMembersPing, String staffMembersGamemode) {
        StaffCorePlugin.staffMembers.addAll(Arrays.asList(staffMembers.split(", ")));
        StaffCorePlugin.playersServerMap.putAll(utils.makeHashMap(staffMembersServer));
        StaffCorePlugin.playersServerPingMap.putAll(utils.makeHashMap(staffMembersPing));
        StaffCorePlugin.playersServerGamemodesMap.putAll(utils.makeHashMap(staffMembersGamemode));
        this.serverCount++;
