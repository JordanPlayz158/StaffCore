...
public class AltsQuery {
    ...
    public static void createAlts(String player, String IP, String uuid) {
        ...
        while (intento < 4) {
            try {
                ...
                StaffCorePlugin.plugin.getServer().getConsoleSender().sendMessage(Utils.chat("&c[&5Staff Core&c] Creating the sc_alts table into the database"));
                intento++;
                throwable.printStackTrace();
            }
        }
    ...
    public static boolean checkAltsTable() {
        ...
        try {
            ...
            StaffCorePlugin.plugin.getServer().getConsoleSender().sendMessage(Utils.chat("&c[&5Staff Core&c] There has been an error with the mysql"));
            StaffCorePlugin.plugin.getServer().getConsoleSender().sendMessage(Utils.chat("&c[&5Staff Core&c] Not able to connect to the Database"));
            ignored.printStackTrace();
        }
        return false;
    }
